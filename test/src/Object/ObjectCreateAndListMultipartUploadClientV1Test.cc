...
protected:
    ObjectCreateAndListMultipartUploadClientV1Test() = default;

    ~ObjectCreateAndListMultipartUploadClientV1Test() override = default;
...
TEST_F(ObjectCreateAndListMultipartUploadClientV1Test, CreateListMultipartUploadTest) {
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        auto upload = cliV2->createMultipartUpload(bkt_name, idx_string, rob_create);
    }
    ...
    for (int t = 0; t < 10; ++t) {
        ...
        auto output = cliV2->listMultipartUploads(bkt_name, input_multipart_list);
...
TEST_F(ObjectCreateAndListMultipartUploadClientV1Test, AbortMultipartUploadTest) {
    ...
    auto upload_1 = cliV2->createMultipartUpload(bkt_name_, "test-obj-001");
    auto upload_2 = cliV2->createMultipartUpload(bkt_name_, "test-obj-002");
    ...
    auto output_list_1 = cliV2->listMultipartUploads(bkt_name_, input_multipart_list);

    EXPECT_EQ(output_list_1.isSuccess(), true);
    EXPECT_EQ(output_list_1.result().getUpload().size(), 2);

    auto output_abort_1 = cliV2->abortMultipartUpload(bkt_name_, input_1);
    EXPECT_EQ(output_abort_1.isSuccess(), true);
    auto output_abort_2 = cliV2->abortMultipartUpload(bkt_name_, input_2);
    EXPECT_EQ(output_abort_2.isSuccess(), true);

    auto output_list_2 = cliV2->listMultipartUploads(bkt_name_, input_multipart_list);
...
TEST_F(ObjectCreateAndListMultipartUploadClientV1Test, ListObjectWith3PrefixObjectsTest) {
    ...
    auto upload_1 = cliV2->createMultipartUpload(bkt_name_, obj_name_1);
    std::string obj_name_2 = "11/22/34/4444";
    auto upload_2 = cliV2->createMultipartUpload(bkt_name_, obj_name_2);

    ListMultipartUploadsInput input_multipart_list;
    input_multipart_list.setMaxUploads(10);
    input_multipart_list.setPrefix("11/22/");
    input_multipart_list.setDelimiter("/");
    auto output = cliV2->listMultipartUploads(bkt_name_, input_multipart_list);
    EXPECT_EQ(output.isSuccess(), true);
...
TEST_F(ObjectCreateAndListMultipartUploadClientV1Test, CreateUploadsFromNonexistentBucketTest) {
    ...
    auto output = cliV2->createMultipartUpload(nonexistent_bkt_name, obj_name);
...
TEST_F(ObjectCreateAndListMultipartUploadClientV1Test, ListUploadsFromNonexistentBucketTest) {
    ...
    ListMultipartUploadsInput input_multipart_list;
    input_multipart_list.setMaxUploads(10);
    auto output = cliV2->listMultipartUploads(nonexistent_bkt_name, input_multipart_list);
    EXPECT_EQ(output.isSuccess(), false);
    EXPECT_EQ(output.error().getStatusCode(), 404);
    EXPECT_EQ(output.error().getMessage() == "The specified bucket does not exist.", true);
}
TEST_F(ObjectCreateAndListMultipartUploadClientV1Test, AbortUploadsFromNonexistentBucketTest) {
    ...
    auto upload_1 = cliV2->createMultipartUpload(bkt_name, obj_name_create);
    std::string bkt_name_ = TestUtils::GetBucketName(TestConfig::TestPrefix);
    ...
    auto output_1 = cliV2->abortMultipartUpload(bkt_name_, input_multipart_abort);
    ...
    auto output_2 = cliV2->abortMultipartUpload(bkt_name_, input_multipart_abort);
    EXPECT_EQ(output_2.isSuccess(), false);
    EXPECT_EQ(output_2.error().getStatusCode(), 404);

    auto output_3 = cliV2->abortMultipartUpload(bkt_name_, input_multipart_abort);
