...
class GetSymlinkV2Input : public GenericInput {
public:
    GetSymlinkV2Input(std::string bucket, std::string key) : bucket_(std::move(bucket)), key_(std::move(key)) {
    }
    GetSymlinkV2Input(std::string bucket, std::string key, std::string versionId)
            : bucket_(std::move(bucket)), key_(std::move(key)), versionID_(std::move(versionId)) {
    }
    GetSymlinkV2Input() = default;
    ~GetSymlinkV2Input() = default;
