...
public:
  ...
  ~UploadPartInput() = default;
  UploadPartInput(std::string key, std::string uploadId,
                  int64_t partSize, int partNumber,
                  std::shared_ptr<std::iostream> content)
      : key_(std::move(key)), uploadID_(std::move(uploadId)), partSize_(partSize),
        partNumber_(partNumber), content_(std::move(content)) {}
  ...
  void setContent(std::shared_ptr<std::iostream> &content) { content_ = content; }
