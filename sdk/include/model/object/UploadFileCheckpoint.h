...
public:
    ...
    const UploadFileInfo& getFileInfo() const {
        return fileInfo_;
    }
    void setFileInfo(const UploadFileInfo& fileInfo) {
        fileInfo_ = fileInfo;
    }
    ...
    void setUploadFilePartInfoList(const std::vector<UploadFilePartInfo>& uploadFilePartInfoList) {
    ...
    void setUploadFilePartInfoByIdx(const UploadFilePartInfo& uploadFilePartInfo, int idx) {
        if (idx < 0 || idx >= uploadFilePartInfoList_.size())
            return;
    ...
    bool isValid(int64_t uploadFileSize, int64_t uploadFileLastModifiedTime, const std::string& bucket,
                 const std::string& objectKey, const std::string& uploadFilePath);
