...
public:
  RequestBuilder(std::shared_ptr<Signer> signer,
                 std::string scheme, std::string host,
                 std::string bucket, std::string object,
                 int urlMode,
                 std::map<std::string, std::string> headers,
                 std::map<std::string, std::string> query)
      : signer_(std::move(signer)), scheme_(std::move(scheme)), host_(std::move(host)), bucket_(std::move(bucket)),
        object_(std::move(object)), URLMode_(urlMode), headers_(std::move(headers)), query_(std::move(query)) {}
