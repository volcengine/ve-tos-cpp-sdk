...
void VolcengineTos::UploadFilePartInfo::load(const std::string &info) {
  ...
  if (j.contains("Offset")) j.at("Offset").get_to(offset_);
  if (j.contains("PartNum")) j.at("PartNum").get_to(partNum_);
  if (j.contains("PartSize")) j.at("PartSize").get_to(partSize_);
  if (j.contains("IsCompleted")) j.at("IsCompleted").get_to(isCompleted_);
}
