...
void VolcengineTos::UploadPartOutput::load(const std::string &output) {
  ...
  if (j.contains("PartNumber")) j.at("PartNumber").get_to(partNumber_);
  if (j.contains("ETag")) j.at("ETag").get_to(etag_);
  if (j.contains("SSECustomerAlgorithm")) j.at("SSECustomerAlgorithm").get_to(sseCustomerAlgorithm_);
  if (j.contains("SSECustomerMD5")) j.at("SSECustomerMD5").get_to(sseCustomerMD5_);
}
