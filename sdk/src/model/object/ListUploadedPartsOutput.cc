...
void VolcengineTos::ListUploadedPartsOutput::fromJsonString(const std::string &input) {
  ...
  if (j.contains("PartNumberMarker")) j.at("PartNumberMarker").get_to(partNumberMarker_);
  if (j.contains("NextPartNumberMarker")) j.at("NextPartNumberMarker").get_to(nextPartNumberMarker_);
  if (j.contains("MaxParts")) j.at("MaxParts").get_to(maxParts_);
  if (j.contains("IsTruncated")) j.at("IsTruncated").get_to(isTruncated_);
  if (j.contains("StorageClass")) j.at("StorageClass").get_to(storageClass_);
  if (j.contains("Owner")) {
