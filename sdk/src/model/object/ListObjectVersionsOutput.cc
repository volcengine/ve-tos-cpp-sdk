...
VolcengineTos::ListedDeleteMarkerEntry parseListedDeleteMarkerEntry(const json& marker) {
    ...
    if (marker.contains("LastModified"))
        entry.setLastModified(marker.at("LastModified").get<std::string>());
...
VolcengineTos::ListedObjectVersion parseListedObjectVersionV2(const json& version) {
    ...
    if (version.contains("LastModified"))
        listedObjectVersion.setLastModified(version.at("LastModified").get<std::string>());
    ...
    if (version.contains("StorageClass"))
        listedObjectVersion.setStorageClass(version.at("StorageClass").get<std::string>());
    if (version.contains("Type"))
        listedObjectVersion.setType(version.at("Type").get<std::string>());
...
void VolcengineTos::ListObjectVersionsOutput::fromJsonString(const std::string& input) {
    ...
    if (j.contains("Versions")) {
        ...
        for (auto& v : versions) {
            versions_.emplace_back(parseListedObjectVersionV2(v));
        }
    }
