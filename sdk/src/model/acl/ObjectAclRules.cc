...
std::string VolcengineTos::ObjectAclRules::toJsonString() {
    ...
    for (auto& g : grants_) {
        ...
        if (!g.getGrantee().getType().empty())
            grant["Grantee"]["Type"] = g.getGrantee().getType();
        if (!g.getGrantee().getUri().empty())
            grant["Grantee"]["Canned"] = g.getGrantee().getUri();
        if (!g.getPermission().empty())
            grant["Permission"] = g.getPermission();
