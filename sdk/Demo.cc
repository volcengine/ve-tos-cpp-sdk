...
int main(){
  std::string endpoint("https://tos-cn-boe.volces.com");
  std::string region("cn-boe");
  std::string ak("AKLTN2QyNjU4ZDVkZjlkNGMyZjhhYzQxMjBkMjI3YWNhYzM");
  std::string sk("WkRKbVlqZGtaVEk1WWpnNE5ETTJOR0ZrTVRFeFptRTVOemhsTTJObE16WQ==");
  std::string bucket("cyj-sdk-test");
  std::string key("test11");
  InitializeClient();
  TosClient client(endpoint, region, ak, sk);
  auto pa = std::make_shared<TosClient>(client);
  std::thread t1(testMultiThread, pa, bucket, key);
  std::thread t2(testMultiThread, pa, bucket, key);
  std::thread t3(testMultiThread, pa, bucket, key);
  std::thread t4(testMultiThread, pa, bucket, key);
  std::thread t5(testMultiThread, pa, bucket, key);
  std::thread t6(testMultiThread, pa, bucket, key);
  std::thread t7(testMultiThread, pa, bucket, key);
  std::thread t8(testMultiThread, pa, bucket, key);
  std::thread t9(testMultiThread, pa, bucket, key);
  t1.join();
  t2.join();
  t3.join();
  t4.join();
...
//  TosClient client(endpoint, region, ak, sk);
//  //  putObject(&client, bucket, key);
//  std::thread t1(listObjects, &client, bucket);
//  std::thread t2(listObjectVersion, &client, bucket);
//  std::thread t3(listBucket, &client);
//  std::thread t4(listObjects, &client, bucket);
//  std::thread t5(listObjectVersion, &client, bucket);
//  std::thread t6(listBucket, &client);
//  std::thread t7(listObjects, &client, bucket);
//  std::thread t8(listObjectVersion, &client, bucket);
//  std::thread t9(listBucket, &client);
//  t1.join();
//  t2.join();
//  t3.join();
//  t4.join();
